...
// Использование: этот файл заменяет mesh P2P на SFU архитектуру
// Преимущества: клиент отправляет ОДИН поток вместо N, сервер раздает остальным
...
// Создание или получение router для комнаты
async function getOrCreateRouter(roomId) {
  if (routers.has(roomId)) {
    return routers.get(roomId);
  }
  
  const worker = workers[0]; // Используем первый worker
...
// Mediasoup соединения для каждого клиента
const clientMediasoup = new Map(); // wsId -> { router, transport, producer, consumers }
...
function checkAllReady() {
  ...
  if (allReady && !gameState.started) {
    ...
    console.log("🎮 Игра началась! Генерируем карты и устанавливаем видеосвязь...");
...
// Обработка WebRTC через Mediasoup + логика игры
  ...
  ws.on("message", async (message) => {
    try {
      ...
      switch (data.type) {
        ...
        // Mediasoup: создание transport
        case "createTransport": {
          const { roomId, direction } = data; // "send" или "recv"
          const router = await getOrCreateRouter(roomId);
          
          const transport = await router.createWebRtcTransport({
            listenIps: [
              { ip: '0.0.0.0', announcedIp: '87.228.76.59' } // Ваш внешний IP
            ],
            enableUdp: true,
            enableTcp: true,
            preferUdp: true,
          ...
          transport.on('iceconnectionstatechange', (iceConnectionState) => {
            console.log(`ICE состояние (${direction}): ${iceConnectionState}`);
          });
        ...
        // Mediasoup: подключение transport
        case "connectTransport": {
          const { direction, dtlsParameters } = data;
          const clientData = clientMediasoup.get(ws.id);
          
          if (!clientData || !clientData.transports.has(direction)) {
            ws.send(JSON.stringify({ type: "error", message: "Transport не найден" }));
            return;
          }
        ...
        // Mediasoup: создание producer (отправка видео)
        case "createProducer": {
          const { roomId, kind, rtpParameters } = data;
          const clientData = clientMediasoup.get(ws.id);
          
          if (!clientData || !clientData.transports.has('send')) {
            ws.send(JSON.stringify({ type: "error", message: "Send transport не найден" }));
            return;
          }
        ...
        // Mediasoup: создание consumer (получение видео)
        case "createConsumer": {
          const { roomId, producerId, rtpCapabilities } = data;
          const clientData = clientMediasoup.get(ws.id);
          
          if (!clientData || !clientData.transports.has('recv')) {
            ws.send(JSON.stringify({ type: "error", message: "Recv transport не найден" }));
            return;
          }
          ...
          if (!router.canConsume({ producerId, rtpCapabilities })) {
            ws.send(JSON.stringify({ type: "error", message: "Несовместимые capabilities" }));
            return;
          }
        ...
        // Mediasoup: получение существующих producers
        ...
        // Mediasoup: возобновление consumer
        case "resumeConsumer": {
          const clientData = clientMediasoup.get(ws.id);
          if (!clientData) {
        ...
        // ИНСТРУКЦИЯ: Скопируйте ВСЕ обработчики игры из websocket.js (строки 932-1736)
        // Вставьте их здесь перед default case
        // Это включает: join_admin_panel, join, set_ready, get_lobby_state,
        // chat_message, get_player_cards, reveal_characteristic, execute_action_card,
        // toggle_ban_player, kick_player, set_mirror_camera, game_ready,
        // set_total_rounds, change_round, start_voting_selection, set_voting_candidates,
        // confirm_voting_candidates, cancel_voting, vote_to_kick, toggle_player_highlight, reset_game
        // 
        // ВАЖНО: Замените в этих обработчиках:
        // - ws.send(JSON.stringify(...)) на sendResponse(...) для ответов с requestId
        // - broadcast() и sendPlayersUpdate() остаются без изменений
